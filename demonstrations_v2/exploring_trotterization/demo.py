...
###############################################################################
# As expected, the 'T'-count is much lower in the phase gradient method, even when factoring in 1 Toffoli = 4 T-gates [#Gidney2018]. This, however, is not a definitive affirmation that the phase gradient method is the ideal gate synthesis method here. Let's compare the trotter error in the two methods.
...
# Fast-Fowarding
# --------------
# As demonstrated, implementing quantum simulation algorithms on hardware requires an that a series of gates are implemented for each time step, meaning that the depth of the circuit grows with increasing time. If the depth of a simulation circuit maintains proportionality to the length of the time interval being simulated, for example, it runs the (very real) risk of exceeding the coherence time of the system. In a completely ideal case, running a simulation for time :math:`t` would require a complexity less than :math:`\mathcal{O}(t)` or, in other words, a complexity that is sublinear in :math:`t`. This, in theory, can be achieved by employing a "fast-forwarding" technique, which is an umbrella term that refers to strategies used to reduce the depth of a simulation circuit below the time step threshold.
#
# The applications of the specific strategies contined under this umbrella is decided based on the structure of the Hamiltonian being simulated itself. If your system has an analytical solution, your life is easy. Fast-forwarding in the analytical case can be carried out by computing the unitary transformation that diagonalizes the system's Hamiltonian and executing exactly using quantum gates. Since the entire evolution of the system is known, the phase angles used in the time-evolution operator (as shown above) can be altered to obtain different time lengths, essentially carrying out simulation with :math:`\mathcal{O}(1)` complexity. This is an incredibly ideal result that applies to a select few, generally not very interesting scenarios, such as a fixed pendulum. 
#
