...
###############################################################################
# From this estimate, we can see that even a simple, single pass of a rotation multiplexer for small set of arbitrary angles requires over 300 T-gates. Considering the cost of distillation and the fact that useful applications of this procedure will need to handle many more qubits and many more rotations, it is easy to see that the cost will grow very quickly. We need a hero!
#
...
#
# The phase gradient state basically acts as a conditional rotation operator in itself. What is special, however, is that this state can be prepared once, stored in a register, then *added* to a data register (like the control register we discussed previously) to induce the desired phase shift on a bit in a data register via `phase kickback <https://pennylane.ai/codebook/quantum-phase-estimation/catch-the-phase>`_. One can take the term "gradient" literally here; the phase gradient state essentially stores spatially dependent phases that can be applied to encode data as a function of qubit position. More technically, when an integer is added to the phase gradient register, the spatially dependent phases encoded in the gradient structure will result in each qubit in the gradient register picking up a phase proportional to the integer value. The phase kickback process will then assign the phase associated with the qubit position to the original data qubit, successfully loading the desired state. There are two main optimizations that emerge from this strategy. First, the phase gradient states only needs to be generated *once* since its catalytic nature leaves it unchanged after it interacts with the data register, meaning it will have a one-time, upfront preparation T-gate cost that is never repeated. Second, the phase shifts are applied by an addition operation rather than multiplication, meaning the phase gradient method's T-gate count scales as :math:`\mathcal{O}(n+\log_2(1/\epsilon))`. 
#
# ADDITION ANIMATION?
#
