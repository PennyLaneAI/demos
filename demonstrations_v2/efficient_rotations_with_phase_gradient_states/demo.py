...
.. admonition:: Quantifying Efficiency
   ...
   All quantum circuits can be broken down into a set of discrete gates (`Clifford gates <https://pennylane.ai/qml/demos/tutorial_clifford_circuit_simulations>`_) and a choice of a continuous gate, such as :math:`R_Z`. This combination is said to be `universal <https://pennylane.ai/qml/demos/tutorial_achieving_universality_with_the_clifford_hierarchy>`_, meaning we can approximate any valid quantum computational process using a sequence of the members of this set. Arbitrary rotations, for example, can be represented by a combination of Clifford + non-Clifford rotation gates.

   |

   The `T gate <https://pennylane.ai/compilation/clifford-t-gate-set>`_, which represents a fixed :math:`\frac{\pi}{4}` rotation is a popular choice of continuous gate. To approximate some rotation using Clifford + T gates, the rotation can be synthesized from a gate sequence that becomes more complex the more the desired rotation deviates from the :math:`\frac{\pi}{4}` rotation induced by a singular T gate. 
 
   .. figure:: ../demonstrations_v2/efficient_rotations_with_phase_gradient_states/GateDecomp.gif
        :align: center
        :width: 300px

   The point? It takes **a lot** of T gates to replicate arbitrary rotations within the universal gate set, and T gates themselves are resource intensive to produce. Via a process called `magic state distillation <https://pennylane.ai/qml/demos/tutorial_magic_state_distillation>`_, the quality of a T gate used in a circuit implementation is improved through a redundant process that typically ends up consuming several T gates to reach the final copy. When one considers the expense of T gate generation, distillation, *and* rotation synthesis, it quickly becomes clear why we want to design systems that use as few gates as possible to maintain feasibility. 
...
Efficient Rotations?
--------------------
Arbitrary rotations are expensive. Unfortunately, they are also foundational to some of the most important tools we have in quantum algorithms and computing, such as the `quantum Fourier transform (QFT) <https://pennylane.ai/qml/demos/tutorial_qft>`_, `quantum phase estimation <https://pennylane.ai/demos/tutorial_qpe/>`_, and the `Trotterization algorithm <https://pennylane.ai/challenges/a_simple_trotterization>`_. As a result, optimizing these operations in the fault-tolerant picture is very important. There have been a diverse array of optimization strategies that jockey for the position of being the most effective at reducing the number of T gates required to carry out a rotation. 

To emphasize the benefit of T gate optimization, let us take the example of time evolution simulation on a computational grid. Here, a wavefunction discretized to :math:`N=2^n` grid points, where :math:`n` is the number of qubits required to represent the dimensions of the grid, can be effectively evolved in time via the application of position-dependent phase shifts represented as controlled rotations. In the naïve approach, in which each point on the grid is treated independently, the system's gate count will scale as :math:`\mathcal{O}(2^n\log_2(1/\epsilon))`, where :math:`\epsilon` is the desired precision. Yikes.
