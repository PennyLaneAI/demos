...
Upon measuring the top two qubits, like in standard state teleportation, a Pauli error $P$ occurs on the bottom qubit. There is an equal chance of :math:`\{X,Y,Z,I\}` occurring. For state teleportation, applying :math:`P^{\dagger}=P` removes that error. However, the difference here with gate teleportation is that Bob applied $U$ already. So, after measurement, the bottom qubit becomes :math:`UP|\psi\rangle`. If any :math:`P\neq I` occurred, then applying :math:`P^{\dagger}` does not remove the Pauli error. 

To see how we can overcome this, let's apply the identity :math:`I=U^{\dagger} U` to obtain :math:`UPU^{\dagger}U|\psi\rangle = CU|\psi\rangle`. By the Clifford hierarchy, $C$ must be a Clifford gate. As discussed above, many QEC codes can implement Clifford gates fault-tolerantly. Thus, with the knowledge of $P$ from the Bell state measurement, :math:`C^{\dagger} = UPU^{\dagger} = C` can be applied to produce :math:`U|\psi\rangle`, the desired non-Clifford gate. This procedure, known as magic state injection, generalises to the n-qubit case. 
