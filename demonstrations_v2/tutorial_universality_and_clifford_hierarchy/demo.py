...
But to achieve [fault-tolerant universal quantum computing](https://pennylane.ai/topics/fault-tolerant-quantum-computing), quantum states must be encoded with [quantum error correction (QEC) codes](https://pennylane.ai/codebook/quantum-error-correction). Many QEC codes such as the `CSS <https://pennylane.ai/qml/demos/tutorial_stabilizer_codes>`__, colour, surface, and qLDPC [link to Utkarsh's upcoming demo] codes have transversal implementations of Clifford gates. Transversal gates limit the propagation of errors, which can easily provide fault-tolerant gates. 
...
Examples of members of this group include the :math:`T = \sqrt{S}` gate, the Toffoli gate, and :math:`\mathrm{CCZ}` gates. For example, the $T$ gate conjugates Pauli gates like so: :math:`TXT^{\dagger} = e^{-i \pi/4} SX \~ SX` and :math:`TYT = -e^{-i\pi/4} XS \~ XS` up to a global phase. 
...
So, as shown in Figure 1, suppose we apply a gate :math:`U\in \mathcal{C}_3` on Bob’s half of the Bell state pair on the bottom, and proceed with :math:`|\psi\rangle` teleportation as usual. We won't worry about how Bob can apply $U$ but Alice can't just yet. Suffice to say, it's a *magic* state :magic_wand:.
...
One can inject a $T$ gate via the circuit presented in Figure 3c, or using the circuit below. Additional explanation of the circuit below can be found [here](https://pennylane.ai/qml/glossary/what-are-magic-states). 
