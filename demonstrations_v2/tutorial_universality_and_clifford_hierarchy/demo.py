...
But to achieve `fault-tolerant quantum computing <https://pennylane.ai/topics/fault-tolerant-quantum-computing>`__, quantum states must be encoded with `quantum error correction (QEC) codes <https://pennylane.ai/codebook/quantum-error-correction>`__. Many QEC codes such as the `CSS <https://pennylane.ai/qml/demos/tutorial_stabilizer_codes>`__, colour, surface, and qLDPC [link to Utkarsh's upcoming demo] codes have transversal implementations of Clifford gates. Transversal gates limit the propagation of errors, which can easily provide fault-tolerant gates. 
